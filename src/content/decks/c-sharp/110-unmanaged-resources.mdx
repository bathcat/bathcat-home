...
```csharp title='ComAdapter.cs'
    /// <remarks>
    /// Called by the user explicly-- or with a `using` block
    /// </remarks>
    public void Dispose() {
        // Delegate to Dipose(bool)
        Dispose(true);
...
~ComAdapter() {
    ...
    Debug.WriteLine("WARNING: ComAdapter finalizer called."+
      " Resource haven't been properly disposed."
    );
...
* [Red functions](https://ericlippert.com/2015/11/30/red-green-refactor/) are dangerous to call
  - Can throw exceptions
  - Can modify state
  - Can have side effects
* Blue functions are safe to call
  - Pure functions
  - No side effects
  - Always return same output for same input
* Finalizers must be blue functions
  - Can't throw exceptions
  - Can't access managed resources
  - Must be thread-safe
