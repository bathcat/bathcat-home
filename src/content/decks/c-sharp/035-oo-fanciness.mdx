...
description: "A deep dive into advanced object-oriented programming concepts in C#, including inheritance, virtual methods, and System.Object fundamentals."
status: 'Live'
flavor: 'Deck'
---
...
2. Describe the difference between `virtual` and `override`
3. Define the concept of an `abstract` class
4. Name 2 methods that every class has 
...
### Composition
* [Composition](https://en.wikipedia.org/wiki/Object_composition) means including objects as members
* Aka 'has-a' relationship
* Benefits:
  - Code reuse _without_ tight coupling
  - Relationships can change at runtime
  - Easier to test and modify
* Example: `Car` has-a `Engine` instead of is-a `Engine`
* **Slogan**: ["Favor composition over inheritance"](https://en.wikipedia.org/wiki/Composition_over_inheritance)
...
### Explanation
* `Button` **is a** `Control`
* Reuse `Control`'s implementation of `RequestRepaint`
* Override `Paint` and `OnMouseDown` to customize behavior
* Client code gets a uniform interface
...
* [Design philosophy](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/virtual-members):
  ...
  - Prevents ["fragile base class"](https://en.wikipedia.org/wiki/Fragile_base_class) problem
