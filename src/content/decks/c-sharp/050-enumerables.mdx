---
title: "Enumerables"
description: "Overview of C# enumeration concepts, covering the iterator pattern, LINQ operations, collection types, and modern syntax features that enable efficient data processing and manipulation."
status: 'Live'
flavor: 'Deck'
---
...
1. Explain the iterator pattern
2. Describe how to use the `yield` keyword
3. Explain when to use arrays vs lists
4. Describe the syntax of collection expressions
...
// The IEnumerator generic interface
public interface IEnumerator<out T> : IDisposable, IEnumerator{
    T Current { get; }  // Returns current element
}

// The non-generic version
public interface IEnumerator{
    object? Current { get; }  // Returns current element as object
    bool MoveNext();         // Advances to next element
    void Reset();            // Resets position to before first element
}
```
...
var query = numbers
    ...
    .Where(n => { Console.WriteLine($"Filtering {n}"); return (n % 2) == 0; })    
...
* **Deferred execution**: Changes to source data [effect results](https://learn.microsoft.com/en-us/dotnet/standard/linq/deferred-execution-example)
...
### What to pick?
* **Choose Array when:**
  - [Size is fixed](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/arrays/#array-overview) and known at creation
  - Maximum performance is critical
  - Working with [interop scenarios](https://learn.microsoft.com/en-us/dotnet/framework/interop/marshaling-data-with-platform-invoke)

* **Choose List when:**
  - Collection [needs to grow](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.capacity) dynamically
...
### New Hotness: Collection Expressions
* Introduced in [C# 12/.NET 8](https://learn.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-12#collection-expressions)
* [Uniform syntax](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/collection-expressions) for creating arrays, lists, and more
* Syntax:
  - Square brackets
  - Spread operator `..`
  - Infers the most appropriate collection type

```csharp
int[] array = [1, 2, 3];                      // Array creation
List<string> combined = ["a", .. otherList];  // Combining collections
```
