---
title: "Functions"
description: "Covers C#'s function-related features: extension methods, local functions, lambdas, and delegates provide different approaches to code organization and functional programming patterns."
status: 'Live'
flavor: 'Deck'
---
...
1. Describe the syntax for writing an extension method
2. Distinguish between `IFunc` and `Action`
3. Define the term "higher-order function"
4. Explain how to create a simple lambda expression
...
public static class StringHelper{
    ...
    public static string AddDot(char initial)
        => initial == ' ' ? "" : $"{initial}. ";
}

public static string FormatName(string first, string last){
    // Kind of ugly to use
    var initial = StringHelper.AddDot(StringHelper.GetInitial(first));
    return $"{initial}{last}";
}
...
public static class StringExtensions{
    // Notice `this`
    public static char GetInitial(this string name)
        => name[0] ?? '';

    // Here too
    public static string AddDot(this char initial)
        => initial == ' ' ? "" : $"{initial}. ";
}

public static string FormatName(string first, string last)
    => $"{first.GetInitial().AddDot()}{last}";
...
### What's the Point?
* **Ergonomics** 
    - Calling instance-like methods feels natural
    - Better discoverability via IntelliSense
    - Enables fluent interfaces with method chaining
* **Flexibility** 
    - "Extends" types you can't modify (sealed classes, types from other libraries)
    - Keeps related functionality grouped together
* **No actual magic** 
    - Just static methods with syntactic sugar
...
internal class Program{
    static int Main(){
        ...
        var ys = xs.Tap(x => Console.WriteLine($"Processing {x}"))
                   .Select(x => x * 2)
                   .Tap(x => Console.WriteLine($"After Select: {x}"))
                   .ToList();
        return 0;
    }
}
...
### [Best Practices](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/extension-methods)
* [**Prefer** static classes](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/extension-methods#extending-predefined-types) for extension methods
* Think about the [namespace](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/names-of-namespaces):
    - Consider a dedicated namespace like `MyLibrary.Routing`
    - Consider using the same namespace as the type you're extending
* Avoid signature duplication
* Avoid extension methods on `System.Object`
...
### Overview
* [Local functions](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/local-functions) are methods defined inside other methods
* Introduced in C# 7.0
* Only accessible within the containing method
* Benefits:
  - Limit scope/visibility of helper methods
  - Can [access variables](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/local-functions#local-functions-and-variables) from containing scope
  - Better [performance](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/local-functions#local-function-vs-lambda-expression) than lambdas (no heap allocation)
  - Can be [static](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/local-functions#static-local-functions) to prevent capture
...
static int Main(){
    ...
    try{
        WriteLineGreen("Begin enfarculation");
        // etc...
        WriteLineGreen("End enfarculation");
    }
    catch (Exception ex){
...
### Pros and Cons
* **Pros**:
  - Clean API surface clean and minimal
  - Clear intention
  - Improves readability (functionality near where it's used)
  - Encapsulates implementation details
  - No need to worry about external consumers
* **Cons**:
  - Not reusable across methods
  - Not testable
...
* [Two forms of lambda expressions](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/lambda-expressions#expression-lambdas)
* **Expression lambda**: 
  - **Syntax**: `(parameters) => expression`
  - Single statement, no braces needed
  - Automatically returns result of expression
* **Statement lambda**: 
  - **Syntax**: `(parameters) => { statements; }`
  - One or more statements
  - Braces and semicolons
  - Explicit `return` for non-void lambdas

---

### Example: Expression vs Statement

```csharp
// Expression lambda - concise, returns result directly
Func<int, int> square = x => x * x;

// Statement lambda - multiple statements, explicit return
Func<string, int> wordCount = s => {
