...
* Relies on IDs being unguessable
* Attacker can access any customer if they know the GUID
...
```
* ***This is a DTO failure!***
* Reviews are public; `CustomerId` is not
* Leaks internal references that enable exploitation
* _Not catastrophic by itself_
...
public static async Task<Created<DeliveryReviewDto>> CreateReview(
    IReviewService service,ICurrentUser current,DeliveryReviewDto request) {
    
    if(request.CustomerId == Guid.Empty){
        request.CustomerId = current.CustomerId;
    }
    var saved = await service.CreateReview(request);
    return TypedResults.Created($"{ApiRoutes.Reviews}/{saved.Id}");
}
```

* Attacker supplies `CustomerId` in request body to impersonate others
* **Failure:** Dto includes more properties than users should control

---

### The Fix: One More DTOs
...
```csharp
public static async Task<Created<DeliveryReviewDto>> CreateReview(
    IReviewService service,ICurrentUser current,CreateReviewRequest request) {
    ...
    var saved = await service.CreateReview(review);
    return TypedResults.Created($"{ApiRoutes.Reviews}/{saved.Id}");
}
```
...
| Approach | Examples | AOT Compatible | Pros | Cons |
|----------|----------|----------------|------|------|
| **Reflection-based** | AutoMapper, Mapster | No | Less boilerplate | Runtime overhead, magic |
| **Source generation** | Mapperly, MapsterSourceGen | Yes | Performance, AOT | Setup complexity |
| **Manual mapping** | Custom classes | Yes | Full control, compiler help | More code |
...
public static async Task<Created<DeliveryReviewDto>> CreateReview(
    ...
    IMapper<DeliveryReview, DeliveryReviewDto> mapper,
    ...
    var saved = await service.CreateReview(review);
    return TypedResults.Created($"{ApiRoutes.Reviews}/{saved.Id}", mapper.Map(saved));
}
```
