...
<a href='https://www.youtube.com/watch?v=pVIxO4Z_rYU'>
  <figure  style='width:25%;height:auto;right:0;bottom:0;transform:scaleX(-1);'>
...
* Angular escapes all special characters
* Treats everything as plain text

```typescript title='login-info.component.ts'
@Component({
...
export class LoginInfo {
  @Input()
  contents='<img src="x" onerror="alert(\'SNAKED\')">';
}
```

* Renders literal string, no script execution
...
* `onerror` handler stripped, image displays safely


---

### Unsafe: Raw DOM Manipulation
* Bypass Angular entirely, no automatic sanitization

```typescript title='login-info.component.ts' {9}
@Component({
  selector: 'app-login-info',
  template: `
    <h2>Raw DOM access:</h2>
    <div id='myDiv'></div>`
})
export class LoginInfo implements OnInit {
  ngOnInit(): void {
    document.getElementById('myDiv')!.innerHTML = this.contents;
  }
...
* Script executes - Angular can't protect you here
...
* Examples
  ...
  - Custom font imports
* Use [DOMSanitizer](https://angular.io/api/platform-browser/DomSanitizer) to bypass Angular's protections
...
```typescript title='search-results.component.ts' {12, 15}
...
export class SearchResultsComponent {
  ...
  searchTerm = inject(ActivatedRoute).snapshot.queryParams['search'];

  highlightQuery(text: string): SafeHtml {
    const highlighted = text.replace(
      new RegExp(this.searchTerm, 'gi'),
      match => `<span style="background: linear-gradient(45deg, #ff0, #f0f)">${match}</span>`
    );
...
### What's Wrong Here?
* Developer wants fancy gradient highlighting
* Angular strips inline `style` attributes by default
* So developer bypasses with `bypassSecurityTrustHtml`
* `searchTerm` comes from query string (user-controlled)
* Attacker crafts URL: `?search=<img src=x onerror=alert('pwned')>`
* Victim clicks link, script executes with their session
...
```typescript title='search-results.component.ts'
...
```css title='search-results.component.css'
.search-highlight {
...
* No bypass needed
* Angular allows safe tags and class attributes
...
* Sanitize before storage, not just validate
* Use library like [HtmlSanitizer](https://github.com/mganss/HtmlSanitizer)
...
### Coming Up:Browser-Based Protections
